...
partyrole:
  ...
  enabled: true                          # the common case -- 26 of 26 components currently
                                          # cached in knowledge/components/ declare TMF669, not
                                          # TMF672; set from the spec's own securityFunction, not
                                          # a fixed default (SKILL.md Step 2)

permissionspec:
  image: lesterthomas/permissionspecapi:0.20   # reuse existing image
  versionLabel: permissionspecapi-0.20
  enabled: false                         # only true when the spec genuinely declares TMF672
...
**Port `name` must be ≤15 characters (Kubernetes IANA_SVC_NAME rule) — `helm lint` does NOT catch a violation of this, confirmed by actually running `helm lint` against a chart using the old `http-{{.Release.Name}}-{apishortname}api` pattern below: it passed clean while the rendered Service manifest's port name was 18 characters, which a real `kubectl apply` rejects outright.** Reuse the same short abbreviated name already used for the container's own port (`{abbrapi}`) as the Service's port name too, instead of a separate, longer `http-...` name — it only needs to be unique within that one Service object, so the short form is always sufficient:
...
- If the spec's `securityFunction.exposedAPIs` declares `TMF669` (the common case): `partyrole.enabled=true`, deploy `partyroleapi` image
- If it genuinely declares `TMF672`: `permissionspec.enabled=true`, deploy `permissionspecapi` image
