...
- **OpenAPI spec URLs**: Get them from `exposedAPIs[].specification[0].url` in the specification YAML, or from this repo's own `knowledge/apis/{TMFxxx}/{TMFxxx}_v{version}.json` cache when it's already there (Step 4a). The swagger spec must end up saved as `api/swagger.yaml` in each microservice, in YAML, regardless of which source it came from.
