...
Read every row's `name` in `knowledge/index/use-cases.json` and compare
...
2-3 candidates (`knowledge/use-cases/{ID}/{ID}.md`) to confirm the match
...
`knowledge/index/gaps-backlog.md` — if the requirement matches one of the
...
Check each proposed component's `status` in `knowledge/index/components.json`
...
**Secondary cross-check via eTOM.** Scan `knowledge/etom/processes.json`
for a process whose `name`/`brief_description` matches the requirement's
own verbs/nouns (favour Level 2–4). For any match, read
`knowledge/index/etom-index.json` → `implemented_by[<id>]` for the
...
- Does not invent a component/API id that isn't in `knowledge/index/{components,apis}.json` — an imperfect real analog is always preferable to a fabricated perfect-sounding one.
