...
`knowledge/index/gaps-backlog.md` — if it does match a known gap, the
...
`knowledge/index/{components,apis}.json` first. If a real match exists,
...
id that doesn't trace back to something real in `knowledge/` doesn't
belong in the draft.
...
guessed next-available number. Check `knowledge/index/gaps-backlog.md`
...
registry (`knowledge/index/usecase-list.json`) is the authority on real
ids, not this skill.
