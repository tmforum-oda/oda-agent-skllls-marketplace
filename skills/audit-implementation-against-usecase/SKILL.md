...
Read `knowledge/use-cases/{ID}/{ID}.md` — the `# Description` and
...
exist in the cached schema (`knowledge/apis/{TMFxxx}/{TMFxxx}_v{version}.json`).
...
`knowledge/index/apis.json` for other cached versions of the same id
...
- Implementation calls an API/operation the use case doesn't link — flag
  ...
  frontmatter is incomplete (cross-check `knowledge/index/matrix-discrepancies.md`
