...
simplest skill against this repo's `knowledge/` layout (spec.md §11.1),
...
```
knowledge/index/components.json   -- one row per component, keyed by id
knowledge/index/apis.json         -- one row per (id, version) pair
```
...
Also cross-check `knowledge/index/usecase-component-matrix.json` and
`knowledge/index/matrix-discrepancies.md` for the same id — if the id is
...
For every id in `used_by`, look up that row in `knowledge/index/use-cases.json`
...
- Does not read any use case's document body — only the three
  `knowledge/index/*.json` files and nothing else. If the caller needs to
