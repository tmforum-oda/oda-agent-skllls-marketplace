...
Read `links.apis` from `knowledge/use-cases/{ID}/{ID}.md`'s frontmatter.
Each entry gives an id and name; check `knowledge/index/apis.json` for
...
```
knowledge/apis/{TMFxxx}/{TMFxxx}_v{version}.json       -- the OpenAPI/Swagger schema
knowledge/apis/{TMFxxx}/samples/                        -- real example payloads, if fetched
```
