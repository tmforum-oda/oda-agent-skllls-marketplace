...
against this repo's `knowledge/` layout (spec.md §8.1): it reads only YAML
...
`knowledge/index/usecase-list.json`'s `status_in_ig1228` field if the
...
- Does not query TM Forum's website — everything it needs is already in
  `knowledge/`, per this repo's provenance principle (spec.md principle 4):
