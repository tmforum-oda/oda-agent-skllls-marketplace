...
references, resolve them to a real id via `knowledge/index/{components,apis}.json`
...
- A version assumption — if the design assumes v5 behavior but only v4
  is cached in `knowledge/index/apis.json` for that id (or vice versa),
...
`knowledge/index/gaps-backlog.md` before reporting it as simply wrong —
