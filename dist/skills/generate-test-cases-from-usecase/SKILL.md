...
`${CLAUDE_PLUGIN_ROOT}/knowledge/use-cases/{ID}/media/imageNN.png` (or `.jpeg`), referenced
...
Also check `${CLAUDE_PLUGIN_ROOT}/knowledge/index/usecase-component-matrix.json` for the same
id and cross-reference against `${CLAUDE_PLUGIN_ROOT}/knowledge/index/matrix-discrepancies.md`
...
specific call. If the id's status in `${CLAUDE_PLUGIN_ROOT}/knowledge/index/apis.json` is
...
```gherkin
# Generated from ${CLAUDE_PLUGIN_ROOT}/knowledge/use-cases/{ID}/{ID}.md ({maturity} / {approval_status})
# Component/API ids cited below are taken from this use case's own frontmatter
# links and/or ${CLAUDE_PLUGIN_ROOT}/knowledge/index/usecase-component-matrix.json -- see Sources.
...
- This repo has no `feature-definition-and-test-kit`-style directory of
  ...
  the `.feature` file into `${CLAUDE_PLUGIN_ROOT}/knowledge/`**, which is generated,
...
```gherkin
# Sources:
#   TMFC020 Digital Identity Management -- ${CLAUDE_PLUGIN_ROOT}/knowledge/use-cases/{ID}/{ID}.md links.components
#   TMF632 Party Management v4.0.0      -- ${CLAUDE_PLUGIN_ROOT}/knowledge/apis/TMF632/TMF632_v4.0.0.json
#   TMFC050 -- ${CLAUDE_PLUGIN_ROOT}/knowledge/index/usecase-component-matrix.json only (not in this document's own References section; see matrix-discrepancies.md)
```
...
- Does not assume `oda-canvas`-specific step vocabulary (`Given a running
  ...
  being tested.
- Does not write output files into this repository — `${CLAUDE_PLUGIN_ROOT}/knowledge/` is
