...
```
${CLAUDE_PLUGIN_ROOT}/knowledge/etom/processes.json        -- the corpus, one entry per process id
${CLAUDE_PLUGIN_ROOT}/knowledge/etom/deleted.json          -- process ids removed over eTOM's history
${CLAUDE_PLUGIN_ROOT}/knowledge/index/etom-index.json      -- implemented_by + stale_component_refs
```
...
- **Not found there either** → before concluding, check
  `${CLAUDE_PLUGIN_ROOT}/knowledge/index/etom-index.json`'s `stale_component_refs` for an entry
  ...
  `${CLAUDE_PLUGIN_ROOT}/knowledge/index/etom-anomalies.md`. If there's no stale ref either, say
...
Read `${CLAUDE_PLUGIN_ROOT}/knowledge/index/etom-index.json`:
...
- Does not read `component.yaml` files, use-case documents, or query TM
  Forum's website — `${CLAUDE_PLUGIN_ROOT}/knowledge/etom/` and `${CLAUDE_PLUGIN_ROOT}/knowledge/index/etom-index.json`
