...
**ODA-first, not ODA-only.** Every finding below is about the architecture reaching for a custom construct where ODA already provides a real, cached equivalent — not a demand that everything be ODA. A genuine gap (no existing component/API covers the capability) is not a finding under this skill; check `${CLAUDE_PLUGIN_ROOT}/knowledge/index/gaps-backlog.md` before flagging something as a violation, the same discipline `decompose-requirement-against-oda` Step 6 already applies.
...
For each capability the architecture assigns to a new or custom component, search `${CLAUDE_PLUGIN_ROOT}/knowledge/index/components.json` by name/description for an existing `TMFCxxx` that already owns that same capability. If one exists, that's a finding: the architecture is duplicating ownership ODA already assigns elsewhere. Cite the real component id and name, not just "an ODA component probably covers this." If nothing in the cached corpus covers it, check `${CLAUDE_PLUGIN_ROOT}/knowledge/index/gaps-backlog.md` before concluding — a genuine, already-logged gap is not duplicated ownership, it's the architecture correctly filling a real hole.

## Step 3 — Check for point-to-point coupling an existing API already mediates

For each direct interaction the architecture describes between two components, check `${CLAUDE_PLUGIN_ROOT}/knowledge/index/apis.json` for a cached `TMFxxx` whose purpose matches that exact interaction (e.g. two components directly sharing a database table to exchange order status, where a cached Open API already defines a real order-status resource and event). If a real, cached API already mediates this kind of interaction, flag the architecture's direct/proprietary link as avoidable coupling — cite the specific API id that should mediate instead.
