...
1. Confirm it in `${CLAUDE_PLUGIN_ROOT}/knowledge/etom/processes.json` (an id not there isn't a
   v26.0 process — check `${CLAUDE_PLUGIN_ROOT}/knowledge/etom/deleted.json`, and if it's gone,
   say so and stop; `explain-etom-process` is the skill for that lookup).
2. Read `${CLAUDE_PLUGIN_ROOT}/knowledge/index/etom-index.json` → `implemented_by[<id>]` for the
