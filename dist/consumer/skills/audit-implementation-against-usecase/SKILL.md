...
Read `${CLAUDE_PLUGIN_ROOT}/knowledge/use-cases/{ID}/{ID}.md` for the expected flow — the
...
`${CLAUDE_PLUGIN_ROOT}/knowledge/index/usecase-component-matrix.json`'s `use_cases` object
(IG1228 chapter 2's own forward index for this id). Per
`${CLAUDE_PLUGIN_ROOT}/knowledge/index/matrix-discrepancies.md`'s corpus-wide check, these two
...
`${CLAUDE_PLUGIN_ROOT}/knowledge/index/apis.json` to get its real cached `version`/`path`
(`${CLAUDE_PLUGIN_ROOT}/knowledge/apis/{TMFxxx}/{TMFxxx}_v{version}.json`) rather than guessing
...
cached here (check `${CLAUDE_PLUGIN_ROOT}/knowledge/index/apis.json` for other cached versions
