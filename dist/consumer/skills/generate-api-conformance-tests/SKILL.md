...
Look up the id+version in `${CLAUDE_PLUGIN_ROOT}/knowledge/index/apis.json`. A `status` other than a real fetched version (`not_yet_specified`, `fetch_failed`, or absent) means there's no real schema to test against — say so and stop, don't test against an assumed shape. If no version is given and more than one is cached for this id, ask which, or generate a suite per version rather than silently picking one.

## Step 2 — Extract assertions directly from the schema

Read `${CLAUDE_PLUGIN_ROOT}/knowledge/apis/{TMFxxx}/{TMFxxx}_v{version}.json` (`definitions`, or `components.schemas` for OpenAPI 3-shaped files). For each resource relevant to the conformance check:
