...
4. **Investigated (`tasks-skills-consumer.md` 0.1) — genuinely blocked, not cleared.** `github.com/tmforum-oda/reference-example-components` carries no `LICENSE`/`LICENSE.md` file, GitHub's own repo metadata reports `license: null`, and the `tmforum-oda` org's `.github` community-health repo (which GitHub would otherwise fall back to) is unlicensed too. Under GitHub's own default terms, a public repo with no license file is "all rights reserved" — viewable, but not confirmed reusable, regardless of it being an official TM Forum org repo intended for public Helm distribution. This is a real, unresolved blocker: Phase 4 (vendoring `implement-oda-component`) should not proceed until someone secures explicit permission from TM Forum/the `tmforum-oda` maintainers, not merely because the content is publicly visible on GitHub. Contrast: `github.com/tmforum-oda/oda-canvas` (source of `create-oda-operator`, §7.9) **is** clearly licensed, Apache License 2.0 — confirmed only because §7.9 was deferred anyway (§4.1), not because the license question there needed solving.
...
| `create-oda-component` (external, already built) | Vendor and adapt as new §7.8 `implement-oda-component` — accepted as a named exception to the read-only posture (§6/§6.1); **blocked** on the source repo's missing license (§6.1 point 4, §11) |
...
- **New, from the Phase 2 pilot above**: whether `generate-api-mocks-from-usecase` (already shipped) and the new `generate-api-conformance-tests` (§7.4) have the same sample/schema version-mismatch exposure `generate-implementation-scaffold-from-usecase` found on `TMF679` (§7.3's own note) — neither was audited for this as part of this pilot. Worth a targeted check, and a matching instruction fix in each if the same pattern recurs, before those skills are trusted the way this one now explicitly guards against it.
...
- **Still open, from §6.1/§7.8** (investigated, not resolved — `tasks-skills-consumer.md` 0.1): `github.com/tmforum-oda/reference-example-components` carries no license at all (confirmed: no `LICENSE`/`LICENSE.md` file, GitHub metadata reports `license: null`, the org's community-health repo is unlicensed too). Genuinely blocks `implement-oda-component` (§7.8) — someone needs to secure explicit permission from TM Forum/the `tmforum-oda` maintainers before Phase 4 of `tasks-skills-consumer.md` can vendor its `templates/`/`references/` content, not just note the gap and proceed anyway.
